...
def expandPerspective_IMG_Matrix (img,pts,offset = 5):
    ...
    for i in range (4):
        ...
        new_pts[i][1] = round(new_h[1]/new_h[2])
    # print(new_pts)
    # cv2.circle(img, (int(new_pts[0][0]),int(new_pts[0][1])), 3, (255,0,0), 3) 
    # cv2.imshow("Sss",img)   
    # cv2.waitKey(0)
    m,size = get_MatrixTransform (new_pts) 
    return cv2.warpPerspective(img,m,size),m
def expand_FourPoint(img,pts,offset =5):
    m,size = get_MatrixTransform (pts)
    inverse_m = np.linalg.inv(m)
    offset_pts = [  [0,-offset,1] ,       [size[0],-offset,1],
                    [0,size[1]+offset,1], [size[0],size[1]+offset,1]]
    for i in range (4):
        new_h = np.matmul(inverse_m,np.array(offset_pts[i]))
        pts[i][0] = round(new_h[0]/new_h[2])
        pts[i][1] = round(new_h[1]/new_h[2])
    new_pts = np.zeros((4,2),dtype='float32')
    s = pts.sum(axis = 1)
    new_pts[1] = pts[np.argmin(s)]
    new_pts[3] = pts[np.argmax(s)]
    diff = np.diff(pts,axis = 1)
    new_pts[0] = pts[np.argmin(diff)]
    new_pts[2] = pts[np.argmax(diff)]
    for i in range(4):
        cv2.circle(img, (int(new_pts[i][0]),int(new_pts[i][1])), 3, (255,0,0), 3) 
        cv2.imshow("Sss",img)   
        cv2.waitKey(5000)
    print(new_pts)
    return new_pts
